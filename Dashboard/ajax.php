...
// 	$std_id = $_GET['std_id'];
// 	$ls_id = $_GET['ls_id'];
	$sqll = $gcc->query("SELECT * FROM access_students WHERE id='{$getF}'");
	$row_cnt1 = $sqll->num_rows;
	if ($row_cnt1 > 0) {
	    $countR1 = mysqli_fetch_assoc($sqll);
	    
	    if($countR1['password'] == $pass){
		    $datae= array('error'=>false,'message'=>'Scam don enter','user'=>array('name'=>$countR1['first_name'].' '.$countR1['last_name'],'studentID'=>$countR1['id'],'centerID'=>$countR1['center_id'],'town'=>$countR1['lga'],'age'=>'A','module'=>'1','level'=>'1'));
